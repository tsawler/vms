	...
	public void BuildPage(HttpServletRequest request,
		...
		try {
			...
			theMenu = HTMLFormDropDownList.makeDropDownListSnippet(
					"template_id", 0, crs, "template_id", "template_name", -1,
					"");
			...
			theMenu = HTMLFormDropDownList.makeDropDownListSnippet(
					"access_level_id", 0, crs, "ct_access_level_id",
					"ct_access_level_name", -1, "");
			...
			theMenu = HTMLFormDropDownList.makeDropDownListSnippet(
					"ct_language_id", 0, crs, "ct_language_id",
					"ct_language_name", -1, "");
			...
			theMenu = HTMLFormDropDownList.makeDropDownListSnippet(
					"parent_archive_page_id", 0, crs, "archive_page_id",
					"archive_page_name", 0, "Top Level Page");

			MasterTemplate.searchReplace("$DDLBPP$", theMenu);

		} catch (Exception e) {
			Errors.addError("com.verilion.display.html.admin.AddArchivePage:BuildPage:Exception:"
					+ e.toString());
		}
	}
}