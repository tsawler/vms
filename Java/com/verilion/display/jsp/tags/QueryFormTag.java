...
public class QueryFormTag extends SimpleTagSupport {
	...
	public void doTag() throws JspException, IOException {
		...
		out.print("<div align=\"center\"><fieldset><legend>" + legend
				+ "</legend>");
		out.print("<form name=\"" + formname + "\" action=\"" + action
				+ "\" method=\"" + method + "\">");
		...
		for (int j = 1; j <= i; j++) {
			...
			if (bHasPostedData) {
				if ((sType.equals("text")) || (sType.equals("password"))
						|| (sType.equals("yn"))) {
			...
			if (sType.equalsIgnoreCase("text")) {
				out.print("<input type=\"text\" class=\"" + inputclass
						+ "\" name=\"" + sName + "\" value=\"" + sInitialValue
						+ "\" />");
			} else if (sType.equalsIgnoreCase("password")) {
				out.print("<input type=\"password\" class=\"" + inputclass
						+ "\" name=\"" + sName + "\" value=\"" + sInitialValue
						+ "\" />");
			} else if (sType.equalsIgnoreCase("yn")) {
				out.print("<select class=\"" + inputclass + "\" name=\""
						+ sName + "\"");
