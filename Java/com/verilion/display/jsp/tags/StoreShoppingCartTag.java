      ...
      try {
         ...
         if (pc.getSession().getAttribute("pHm") != null) {
            ...
            if (hm.get("add") != null) {
               ...
               int item_id = Integer.parseInt(hm.get("itemdetail").toString());
               // check to see if it's in the cart already
               if (hmCart.get(item_id + "") != null) {
                  int itemCount = Integer.parseInt(hmCart.get(item_id + "").toString());
                  itemCount++;
         ...
         if (hmCart.size() > 0) {
            ...
            for (Iterator iter = hmCart.entrySet().iterator(); iter.hasNext();) { 
                ...
                if (drs.next()) {
                   theMenu += "<tr><td>"
                      + drs.getString("item_name")
                      + " ("
                      + value
                      + ")</td></tr>";
                   totalPrice = totalPrice + drs.getFloat("item_price") * Float.parseFloat(value);
                }
                drs.close();
                drs = null;
            }
            theMenu += "<tr><td><br />Total in cart: " + fmt.format(totalPrice) + "</td></tr>";
