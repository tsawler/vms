...
public class FaqCategory implements DatabaseInterface {
	...
	public void changeFaqStatus() throws SQLException, Exception {
		...
		try {
			...
			saveBuf.append("set active_yn = ? where faq_categories_id = ?");
			...
			Errors.addError("FaqCategory:changeFaqStatus:SQLException:"
					+ e.toString());
		} catch (Exception e) {
			Errors.addError("FaqCategory:changeFaqStatus:Exception:"
					+ e.toString());
		} finally {
			if (pst != null) {
				pst.close();
	...
	public void deleteFaq() throws SQLException, Exception {
		...
		try {
			...
			saveBuf = new StringBuffer(
					"delete from faq_categories where faq_categories_id = ?");
			...
			Errors.addError("FaqCategory:deleteFaq:SQLException:"
					+ e.toString());
		} catch (Exception e) {
