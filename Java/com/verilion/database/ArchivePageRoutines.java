...
public class ArchivePageRoutines {
	...
	/**
	 * Gets archive page contents, template, and other info. Results are set,
	 * and retreived using assocated getter.
	 * 
	 * @throws SQLException
	 * @throws Exception
	 */
	public void GetArchivePageInfo() throws SQLException, Exception {
		try {
			String query = "select " + "a.archive_page_id, "
					+ "a.archive_page_title, " + "a.archive_page_content, "
					+ "a.archive_page_active_yn, " + "a.ct_access_level_id, "
					+ "t.template_contents, " + "ct_access_level_id, "
					+ "parent_id " + "from " + "archive_page as a, "
					+ "template as t " + "where " + "a.archive_page_name = '"
			...
			e.printStackTrace();
			Errors.addError("ArchivePageRoutines:GetArchivePageInfo:SQLException:"
					+ e.toString());
		} catch (Exception e) {
			e.printStackTrace();
