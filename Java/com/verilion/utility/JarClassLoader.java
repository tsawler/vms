...
public class JarClassLoader extends ClassLoader {
   ...
   public byte[] getClassData(String theDir, String name, String jarName)
      ...
      if (entry != null) {
         try {
            ...
            BufferedInputStream is = new BufferedInputStream(jf
            ...
            if ((entry.toString()).endsWith(".class")) {
               ...
               if (jEntry != null) {
                  try {
                     ...
                     BufferedInputStream is = new BufferedInputStream(jf
                           .getInputStream(jEntry));
                     is.read(byteArray);
                     StringBuffer sb = new StringBuffer(entry.toString());
                     sb.delete((sb.length() - 6), sb.length());
                     defineClass(sb.toString(), byteArray, 0, byteArray.length);
                  } catch (Exception e) {
                     e.printStackTrace();
                  }
               }
    ...
    * @throws IOException
    */
   public synchronized Class LoadModuleClassDataFromJar(
         String jarName,
   ...
   public synchronized Class LoadClass(String name, String jarName)
      ...
      byte[] buf = getClassData(SingletonObjects.getInstance().getSystem_path()
            + "UserJars/", name, jarName);
      if (buf != null)
