...
public class Store {
	 ...
	 * @throws Exception
	 */
	public static String MakeMenuHtml(Connection conn, int categoryId,
			HttpSession session) throws Exception {

		String theMenu = "";
		XDisconnectedRowSet rs = new XDisconnectedRowSet();
		// int cat_level =
		// SingletonObjects.getInstance().getStore_max_subcategories();
		...
		try {
			...
			while (rs.next()) {
				theMenu += "<tr><td>" + "<a href=\"/store/jpage/1/p/Home/cat/"
						+ rs.getInt("category_id") + "/content.do\">"
						+ rs.getString("category_name") + "</a></td></tr>";
				...
				while (crs.next()) {
					theMenu += "<tr><td>&nbsp;&nbsp;- "
							...
							+ crs.getInt("category_id") + "/content.do\">"
							+ crs.getString("category_name") + "</a></td></tr>";
					...
					while (crs2.next()) {
						theMenu += "<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;- "
								+ "<a href=\"/store/jpage/1/p/Home/cat/"
								+ crs2.getInt("category_id") + "/content.do\">"
	 ...
	 * @throws Exception
	 */
	public static String GetItemDetail(Connection conn, int itemId, int cat_id,
			HttpSession session) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				theMenu += "<tr><td valign=\"top\" style=\"width: 180px;\">"
						...
						+ "</td><td valign=\"top\">"
						+ "<a href=\"/store/jpage/1/p/Home" + "/cat/" + cat_id
						+ "/itemdetail/" + rs.getInt("item_id")
						+ "/content.do\"><strong>" + rs.getString("item_name")
						+ "</strong></a><br />"
						...
						+ rs.getInt("item_id") + "/cat/" + cat_id + "/add/1"
						+ "/content.do\">Add to cart</a>" + "</td>"
						+ "</tr></table>";
			}
			rs.close();
			rs = null;
