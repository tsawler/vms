...
public class ResultsTable {
	...
	public String GenerateHTMLResultsTable() throws Exception {
		...
		if ((bPublishButton) || (bUnpublishButton) || (bDeleteButton)
				|| (bUnpublishButton) || (bPublishButton)) {
		...
		try {
					...
					try {
						...
						if (sFieldType[i].equalsIgnoreCase("checkbox")) {
							theTable += "&nbsp;" + sFilterFieldsDisplay[i]
									...
									+ sFilterFields[i] + "\"";
							if (session.getAttribute("filter_"
									+ sFilterFields[i]) != null) {
								theTable += " checked ";
							}
							theTable += ">";
			...
			if (myHm.get("limit") != null) {
				...
				for (int i = 1; i <= iPageCount; i++) {
					if (((i - 1) * Integer.parseInt((String) myHm.get("limit"))) != Integer
						...
						sPager += "<strong>" + "<span style=\"color: red;\">"
								+ i + "</span></strong>" + "&nbsp;";
					}
				}
			...
			theTable += "<form name=\"rt\" action=\"" + sUrl
					+ "\" method=\"post\">";
				...
				for (int i = 0; i <= (sColHeadings.length - 1); i++) {
					try {
						if (bSortable[i] == false) {
							theTable += "<td><strong>" + sColHeadings[i]
									+ "</strong></td>";
						} else {
							theTable += "<td><a href=\"" + sUrl + "?&action=6"
									+ "&colnum=" + i + "\"><strong>"
									+ sColHeadings[i] + "</strong></a></td>";
						}
					} catch (Exception e) {
						theTable += "<td><strong>" + sColHeadings[i]
								+ "</strong></td>";
			...
			if (bPublishColumn) {
				theTable += "<td><strong>" + sPublishedColumnName
						+ "</strong></td>";
			}
			...
			while (rs.next()) {
				...
				theTable += "<td>"
						...
						+ sEditPage + rs.getString(sIdColumn)
						+ "&edit=false\">" + rs.getString(sLinkColName)
						+ "</a></td>";
				...
				if (iNumCols > 0) {
					for (int i = 0; i <= iNumCols - 1; i++) {
						theTable += "<td>" + rs.getString(sColValues[i])
								+ "</td>";
					}
				}
				...
				if (bPublishColumn) {
					...
					if (rs.getString(sPublishCol).equals("y")) {
						theTable += "?action=2" + "&cancelPage=" + sUrl
								+ "&count=1" + "&action_1=1" + "&item_id_1="
								+ rs.getString(sIdColumn) + "\" class=\"pub\" "
								+ "title='" + sPublishTitle + "'>"
								+ sPublishText + "</a>";
					} else {
						theTable += "?action=1" + "&cancelPage=" + sUrl
								+ "&count=1" + "&action_1=1" + "&item_id_1="
								+ rs.getString(sIdColumn)
								+ "\" class=\"upub\" " + "title='"
								+ sUnpublishTitle + "'>" + sUnpublishText
								+ "</a>";
					}
