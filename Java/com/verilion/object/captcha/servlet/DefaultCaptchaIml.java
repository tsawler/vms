...
public class DefaultCaptchaIml implements CaptchaProducer {
	...
	public DefaultCaptchaIml(Properties props) {
		...
		if (this.props != null) {
			...
			if (bbox) {
				boxColor = Helper.getColor(this.props,
						Constants.SIMPLE_CAPTCHA_BOX_C, Color.black);
				boxThick = Helper.getIntegerFromString(props,
						Constants.SIMPLE_CAPTCHA_BOX_TH);
				if (boxThick == 0)
					boxThick = 1;
			}

			this.gimpy = (GimpyEngine) Helper.ThingFactory.loadImpl(
					Helper.ThingFactory.OBSIMP, props);
			this.backGroundImp = (BackgroundProducer) Helper.ThingFactory
					.loadImpl(Helper.ThingFactory.BGIMP, props);
			this.wordRenderer = (WordRenederer) Helper.ThingFactory.loadImpl(
					Helper.ThingFactory.WRDREN, props);
			this.textProducer = (TextProducer) Helper.ThingFactory.loadImpl(
					Helper.ThingFactory.TXTPRDO, props);
		}
	}
	...
	private static BufferedImage rotate(BufferedImage image) {
		...
		BufferedImage transform = new BufferedImage(width, height,
	...
	private void drawBox(Graphics2D graphics) {
		...
		Line2D d2 = new Line2D.Double((double) 0, (double) 0, (double) 0,
				(double) w);
		graphics.draw(d2);

		Line2D d3 = new Line2D.Double((double) 0, (double) 0, (double) w,
				(double) 0);
		graphics.draw(d3);

		d3 = new Line2D.Double((double) 0, (double) h - 1, (double) w,
				(double) h - 1);
		graphics.draw(d3);

		d3 = new Line2D.Double((double) w - 1, (double) h - 1, (double) w - 1,
				(double) 0);
