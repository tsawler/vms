   ...
   public void doPost(HttpServletRequest request, HttpServletResponse response)
      ...
      if (session.getAttribute("userAdminId") != null) {
         try {
            ...
            // now make sure that it's actually a jar file with a valid class in
            // it
            try {
               JarClassLoader myJCL = new JarClassLoader(SingletonObjects
                     .getInstance().getSystem_path()
                     + "UserJars/", ModuleObject.class.getClassLoader());
               Class myClass = myJCL.LoadClass(
                     multi.getParameter("class_name"),
                     filename);
            ...
            response
                  .sendRedirect("/admin/jpage/1/p/ModulesChoice/a/moduleschoice/class/modules/clear/1/limit/10/offset/0/content.do?filter=1");
