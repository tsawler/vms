 ...
 * 
 */
public class LMSServletCourseOfferings extends HttpServlet {
		...
		try {
			...
			try {
				...
				if (course_completed_yn.equalsIgnoreCase("n")) {
					...
					pst.setInt(4, course_offering_id);
					...
					if (action.equalsIgnoreCase("a")) {
						...
						try {
							query = "insert into course_student_progress (customer_id, course_section_id, start_date_time_access, course_offering_id) "
									+ "values (?, ?, CURRENT_TIMESTAMP, ?)";
							pst = conn.prepareStatement(query);
							pst.setInt(1, customer_id);
							pst.setInt(2, course_section_detail_id);
							pst.setInt(3, course_offering_id);
							pst.executeUpdate();
							pst.close();
							pst = null;
						} catch (Exception e) {
						...
						try {
							...
							while (drs.next()) {
								cspid = drs.getInt("id");
								query = "update course_student_progress set end_date_time_access = CURRENT_TIMESTAMP where id = ?";
								pst = conn.prepareStatement(query);
								pst.setInt(1, cspid);
								pst.executeUpdate();
								pst.close();
